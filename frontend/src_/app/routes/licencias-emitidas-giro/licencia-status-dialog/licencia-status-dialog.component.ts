...
@Component({
  selector: 'app-licencia-status-dialog',
  templateUrl: './licencia-status-dialog.component.html',
  styleUrls: ['./licencia-status-dialog.component.scss']
})
export class LicenciaStatusDialogComponent implements OnInit {
    errors = [];
    motivo;
    motivo2;
    fecha_cambio_status;
  ...
  InputChange(e){
    ...
    onSubmit() {
      ...
      this.licenciaService.updateStatus(this.checkoutForm.value,this._data.id).subscribe(
          resp=>{
