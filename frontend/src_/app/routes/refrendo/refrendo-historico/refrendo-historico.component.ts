...
&count=1&outputFormat=application/json&cql_filter=id=${this.datos_form.id_municipio}`
                    ...
                    if (municipio_wfs.features && municipio_wfs.features.length > 0) {
                        ...
                        municipio_wfs.features[0].geometry.coordinates[0].forEach(val => {
                            geom4326.push(proj4('EPSG:32614', 'EPSG:4326', val));
                        });
                        ...
                        this._LicenciaSevice.getInfoLicenciaDatos(this.id2).subscribe(datos => {
                            ...
                            if (data.coordonadas_x && data.coordonadas_y) {
                                ...
                                if (turf.booleanPointInPolygon(turf_point, turf_municipio)) {
                                    ...
                                    this.coordenadas_x = data.coordonadas_x;
                                    this.coordenadas_y = data.coordonadas_y;
    ...
    buscarDir2() {
        ...
        this._mapaService.getGoogleMapsGeo(
            `https://maps.googleapis.com/maps/api/geocode/json?address=${domicilio}, ${this.municipio.nombre}, Cuernavaca&key=AIzaSyDiCLJe6JDKiJEKDhIUKtlrsm3KA_jZRdk`
        ).subscribe(gres => {
    ...
    guardarPoligonoTramiteHistorico(data): Observable<any> {
        console.log("guardarPoligonoTramiteHistorico", data)
