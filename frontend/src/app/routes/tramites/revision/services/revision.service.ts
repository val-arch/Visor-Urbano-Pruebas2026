...
export class RevisionService {
  ...
  getDataRevision(folio){
    ...
    return  this.http.get<any[]>(environment.SERVER_ORIGIN +`revision/${btoa(folio)}`,httpOptions);
  }
  getDataRevisionRefrendo(folio){
    ...
    return  this.http.get<any[]>(environment.SERVER_ORIGIN +`revision/refrendo/${folio}`,httpOptions);
  }
  ...
  uploadDataRevision(folio,data){
    return this.http.post<any>(environment.SERVER_ORIGIN + `revision/update/${btoa(folio)}`, data ,{
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      })
    });  
  ...
  uploadDataRevisionDir(folio,data){
    return this.http.post<any>(environment.SERVER_ORIGIN + `revision/updateDir/${btoa(folio)}`, data ,{
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      })
    });  
  ...
  uploadFile(data,id): Observable<any[]> { 
    return this.http.post<any>(environment.SERVER_ORIGIN + `revision/uploadFiles/${btoa(id)}`, data ,{
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      })
    });  
  ...
  uploadFileHist(data,id): Observable<any[]> { 
    return this.http.post<any>(environment.SERVER_ORIGIN + `revision/uploadFilesHist/${btoa(id)}`, data ,{
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      })
    });  
