...
export class MapaViewComponent implements OnInit {
    ...
    construccionCoords = [];
    isSemadetLayerActive: boolean = false; // true if semadet layer is active
    datosPredio: any = {
        localidad: {},
        colonia: {},
        domicilio: {},
    ...
    //
    camposDiamicosMunicipioConstruccion: any;
    datosDinamicosActualConstruccion = {};
    //
    ...
    measureTooltip: any;
    helpMsg = 'Clic sobre un predio para seleccionarlo';
    startDrawMsg: string = 'Clic para empezar a dibujar';
    continuePolygonMsg: string = 'Clic para continuar dibujando el polígono<br>Doble clic para finalizar';
    continueLineMsg: string = 'Clic para continuar dibujando la línea<br>Doble clic para finalizar';
    ...
    ngOnInit(): void {
        ...
        setTimeout(() => {
            console.log(environment.SERVER_ORIGIN_GEO_SERVER,'env',environment)
            this.createMeasureTooltip();
            this.createHelpTooltip();
            this.municipioControl.disable();
            this.aolmap.instance.on('moveend', (e) => {
                console.log( e.map.getView() );
                console.log("moveend", e.map.getView().getCenter())
                ...
                console.log("center32614", center32614)
                if (this.mapZoom < 19) {
                ...
                if (this.mapZoom >= 10) {
                    this.apivujService.getWFSFeature(
                        `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WFS&request=GetFeature&version=2.0.0&typename=VUJ:base_municipio&count=1&outputFormat=application/json&cql_filter=CONTAINS(geom, POINT (${center32614[0]} ${center32614[1]}))`
                    ).subscribe(response => {
    ...
    seleccionarPoligonoArchivo(poligono) {
        ...
        this.guardarPoligonoArchivo(poligono).subscribe(response => {
            ...
            this.datosPredio.url_minimapa2 = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:mapa_ficha_informativa,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,VUJ:planimetria_predio&cql_filter=INCLUDE;id=${response.id}&BBOX=${bbox[1]},${bbox[0]},${bbox[3]},${bbox[2]}`
            this.datosPredio.url_minimapa = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:MapaJaliscoMapaBase,VUJ:catastro,VUJ:registrotramite&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;id=${response.id}&BBOX=${bbox[1]},${bbox[0]},${bbox[3]},${bbox[2]}`
            this.datosPredio.url_uso = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:planparcialdesarrollourbano32614,VUJ:MapaJaliscoMapaBase,VUJ:catastro,VUJ:registrotramite&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=350&styles=,,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;INCLUDE;id=${response.id}&BBOX=${bbox[1]},${bbox[0]},${bbox[3]},${bbox[2]}`
            ...
            this.getRequisitosMunicipioConstruccion();
    ...
    getRequisitosMunicipioConstruccion(){
        this.apivujService.getRequisitosMunicipioConstruccion(this.municipio.id).subscribe((response: any) => {
            response.data.forEach(element => {
    ...
    ubicar_domicilio(domicilio: string) {
        this.barraIOpen = false;
        let error = false;
        //? this condition is to verify if the input is a number
        if ( /^\d+$/.test( domicilio ) ) {
            const request = `${ environment.SERVER_ORIGIN_GEO_SERVER }/VUJ/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=VUJ%3Aplanimetria_predio&CQL_FILTER=${"clave%3D'" + domicilio}'&maxFeatures=50&outputFormat=application%2Fjson`;
            //? this code made a request to get the coordinates of the searched predio
            this.apivujService.getWFSFeature( request ).subscribe( res => {
                ...
                const coordenadas: any[] = res.features[0].geometry.coordinates[0][0];
                
                const predio = [];

                //? getting the coordinates in EPSG:4326
                coordenadas.forEach(vert => {
                    predio.push(proj4('EPSG:32614', 'EPSG:4326', vert));
                });

                //? validate predio is not empty
                if ( predio.length === 0 ) return;
                let sumLat = 0;
                let sumLng = 0;
                
                // Sumar las coordenadas (latitudes y longitudes)
                predio.forEach(([lng, lat]) => {
                    sumLat += lat;
                    sumLng += lng;
                });
                
                // Calcular el promedio de latitudes y longitudes
                const avgLat = sumLat / predio.length;
                const avgLng = sumLng / predio.length;
                
                // El punto medio
                const puntoMedio = [avgLng, avgLat];
                const [ lng, lat ] = puntoMedio;

                this.apivujService.getReverseGeocode(lat, lng).subscribe(domicilio => {
                    this.datosPredio.domicilio = {};
                    this.datosPredio.calle = "";
                    if (domicilio) {
              
                      this.busquedaDomicilioCoords = [domicilio.geometry.location.lng, domicilio.geometry.location.lat];
                      const punto = turf.point(this.busquedaDomicilioCoords);
                      const addr = domicilio;
                      const domicilioArray = addr.formatted_address.split(",");
              
                      //const formaddr = addr.formatted_address.split(",")[0];
                      const localidad = domicilioArray[2].trim().split(" ");
                      this.datosPredio.domicilio = domicilio;
                      this.datosPredio.calle = domicilioArray[0];
                      this.datosPredio.colonia = {
                          nombre: domicilioArray[1],
                          codigo_postal: localidad[0]
              
                      };
                      this.datosPredio.localidad = {
                          nombre: localidad[1],
                      };
                    }
                });

                //? this code creates the minimap
                ...
                this.datosPredio.vectorial = res.features[0];
                if ( error ) return;
                //? this code center the map on the searched predio
                // this.aolview.instance.fit(
                ...
                this.aolview.instance.setCenter([lng, lat]);
                this.aolview.instance.setZoom(18);
                
                return;
                
                //? this code download other data about predio
                ...
                this.datosPredio.url_minimapa2 = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:mapa_ficha_informativa,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,VUJ:planimetria_predio&cql_filter=INCLUDE;id=${res.features[0].properties.id}&BBOX=${bboxMinimap[1]},${bboxMinimap[0]},${bboxMinimap[3]},${bboxMinimap[2]}`
                this.datosPredio.url_minimapa2 = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:mapa_ficha_informativa,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,VUJ:planimetria_predio&cql_filter=INCLUDE;id=${res.features[0].properties.id}&BBOX=${bboxMinimap[1]},${bboxMinimap[0]},${bboxMinimap[3]},${bboxMinimap[2]}`
                this.datosPredio.url_minimapa = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:MapaJaliscoMapaBase,VUJ:catastro,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;id=${res.features[0].properties.id}&BBOX=${bboxMinimap[1]},${bboxMinimap[0]},${bboxMinimap[3]},${bboxMinimap[2]}`
                this.datosPredio.url_uso = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:planparcialdesarrollourbano32614,VUJ:MapaJaliscoMapaBase,VUJ:catastro_ficha,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=350&styles=,,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;INCLUDE;id=${res.features[0].properties.id}&BBOX=${bboxMinimap[1]},${bboxMinimap[0]},${bboxMinimap[3]},${bboxMinimap[2]}`
            });
        } else {
            // direccion
            this.apivujService.getGeocode(domicilio, this.municipio.nombre).subscribe(res_domicilio => {
                //let res_domicilio = this.getDataDomicilio();
                console.log(res_domicilio);
                const loc = res_domicilio.geometry.location
                const direccion =  res_domicilio.formatted_address.toUpperCase();
                ...
                if (direccion.includes("CUERNAVACA")) {
                  ...
                  this.busquedaDomicilioCoords = [loc.lng, loc.lat];
                  const punto = turf.point(this.busquedaDomicilioCoords);
                    ...
                    // this.aolview.instance.fit(extent, {duration: 1000, zoom: 18});
                    // set center and zoom
    ...
    ubicar_domicilio2(domicilio) {
        if (domicilio.length > 6) {
            this.apivujService.getGoogleMapsGeo(
                `https://maps.googleapis.com/maps/api/geocode/json?address=${domicilio}, ${this.municipio.nombre}, Jalisco&key=.i.`
    ...
    onClickOverMap(event: any): void {
        ...
        let puedeSeleccionarFeature = false;
        this.capasMunicipio.forEach(capa => {
            if(capa['value'] === 'SEMADET') {
                console.log('Encontrado');
                  
            }
        });

        if (this.municipio.id > 0 && this.aolInteraction === 'obtener_info2') {
            //this.initializeWMSInfo();
           //  this.setupPopUpOverlay(event, clickCoord4326);
            const urlsInfo = this.generateUrlsInfo(resolution,clickCoord4326);
            this.processUrlsInfo(urlsInfo);
        }
        ...
        if (this.mapZoom >= 11) {
                        ...
                        if (response.numberMatched > 0) {
                            ...
                            // if (response.features[0].properties.municipio_id === this.municipio.id) {
                                ...
                                setTimeout(() => {
                                    ...
                                    this.datosPredio.url_zip =    `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WFS&request=GetFeature&version=2.0.0&typename=VUJ:construcciones,VUJ:planimetria_predio2,VUJ:manzanas&outputFormat=shape-zip&format_options=filename:extract.zip&cql_filter=BBOX(geom,${bboxMinimap[0]},${bboxMinimap[1]},${bboxMinimap[2]},${bboxMinimap[3]},%27EPSG:4326%27)`
                                    this.datosPredio.url_minimapa2 = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:mapa_ficha_informativa,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,VUJ:planimetria_predio&cql_filter=INCLUDE;id=${response.features[0].properties.id}&BBOX=${bboxMinimap[1]},${bboxMinimap[0]},${bboxMinimap[3]},${bboxMinimap[2]}`
                                    this.datosPredio.url_minimapa = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:MapaJaliscoMapaBase,VUJ:catastro,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;id=${response.features[0].properties.id}&BBOX=${bboxMinimap[1]},${bboxMinimap[0]},${bboxMinimap[3]},${bboxMinimap[2]}`
                                    this.datosPredio.url_uso = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:planparcialdesarrollourbano32614,VUJ:MapaJaliscoMapaBase,VUJ:catastro_ficha,VUJ:planimetria_predio2&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=350&styles=,,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;INCLUDE;id=${response.features[0].properties.id}&BBOX=${bboxMinimap[1]},${bboxMinimap[0]},${bboxMinimap[3]},${bboxMinimap[2]}`
                                    this.cargandoMunicipio = true;
            ...
            this.aolInteraction === ''
            this.apivujService.getWFSFeature(
                `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WFS&request=GetFeature&version=2.0.0&typename=VUJ:base_municipio&count=1&outputFormat=application/json&cql_filter=CONTAINS(geom, POINT (${clickCoord32614[0]} ${clickCoord32614[1]}))`
            ).subscribe(response => {
        ...
        console.log('aqui termina el click')
    }
    ...
    comercialPoligonoTerminado(event): void {
        ...
        const municipiosbaseurl = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WFS&request=GetFeature&version=2.0.0&typename=VUJ:base_municipio&outputFormat=application/json&cql_filter=`
    ...
    dibujoTerminado(event): void {
        ...
        setTimeout(() => {
            ...
            this.datosPredio.url_minimapa = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:MapaJaliscoMapaBase,VUJ:catastro,VUJ:registrotramite&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=600&styles=,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;id=DIBUJOID&BBOX=${bbox[1]},${bbox[0]},${bbox[3]},${bbox[2]}`
            this.datosPredio.url_uso = `${environment.SERVER_ORIGIN_GEO_SERVER}/ows?service=WMS&version=1.3.0&request=GetMap&FORMAT=image/png8&layers=VUJ:planparcialdesarrollourbano32614,VUJ:MapaJaliscoMapaBase,VUJ:catastro,VUJ:registrotramite&exceptions=application/vnd.ogc.se_inimage&CRS=EPSG:4326&width=600&height=350&styles=,,,VUJ:planimetria_predio&cql_filter=INCLUDE;INCLUDE;INCLUDE;id=DIBUJOID&BBOX=${bbox[1]},${bbox[0]},${bbox[3]},${bbox[2]}`
    ...
    setupPopUpOverlay(event, clickCoord4326) {
        this.popUp = new ol.Overlay({
            element: this.popUps.nativeElement,
            position: event.coordinate,
            autoPan: true,
        ...
        this.popUp.setPosition(clickCoord4326);
        this.aolmap.instance.addOverlay(this.popUp);
    }
    generateUrlsInfo(resolution,clickCoord4326) {
        const urlsInfo = [];
        if (this.SourceWMS.length) {
            this.SourceWMS.forEach(source => {
              
                const layersName = source.instance.getParams().LAYERS.split(":");
                const nombreCapa = layersName.length === 2 ? layersName[1] : layersName[0];
                const urlInfo = source.instance.getGetFeatureInfoUrl(
                    this.clickCoord4326, 
                    resolution, 
                    'EPSG:4326',
                    { 'INFO_FORMAT': 'application/json', FEATURE_COUNT: 10 }
    ...
    generateUrlsInfoimg(resolution, clickCoord4326) {
        const urlsInfo = [];
        if (this.SourceWMS.length) {
            this.SourceWMS.forEach(source => {
                const layersName = source.instance.getParams().LAYERS.split(":");
    ...
    processUrlsInfo(urlsInfo) {
        if (urlsInfo.length) {
            urlsInfo.forEach(url => {
                this.fetchAndProcessResponse(url);
            });
        }
    }   
    fetchAndProcessResponse(url) {
        this.clickCoord4326;
        Swal.fire({
            title: 'Procesando...',
            ...
            allowOutsideClick: false,
            didOpen: () => {
                Swal.showLoading(); // Muestra un loader mientras se procesa la petición
            }
        });
        this.apivujService.getWFSFeature(url[1]).subscribe(response => {
            if (response.numberReturned > 0) {
                Swal.close();
                const properties = response.features.map(feature => feature.properties);
                const bladeUrl = `${environment.SERVER_ORIGIN}ficha_semadet/${properties[0]['clave']}`; 
                window.open(bladeUrl, "_blank"); // Esto abre la URL en una nueva pestaña o ventana.
            }
        });
    }
}
