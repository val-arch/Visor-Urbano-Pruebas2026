...
export class TramiteComponent implements OnInit {
  ...
  */
  ...
  verResumen() {
    ...
    this.route2.navigate([`tramite-giros/resumen/${this.folio}`])
  }
  ...
  checkCopyPropietario(event: any) {
    if (event.checked) {
      this.filterForm_step2.patchValue({
        nombre_propietario:this.nombre_solicitante ,
        domicilio_propietario: this.calle,
        colonia_propietario: this.colonia,
      });
    }else{
      this.filterForm_step2.patchValue({
        nombre_propietario:'',
  ...
  getInfoConsultaRequisitos() {
    this._resumenService.getInfoTramite(atob(this.folio)).subscribe(
      (r: any) => {
        ...
        this.nombre_solicitante = r.data['info'].nombre_solicitante
