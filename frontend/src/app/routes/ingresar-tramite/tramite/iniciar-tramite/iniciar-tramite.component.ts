...
export class IniciarTramiteComponent implements OnInit {
  ...
  folioValidado(id, type){
    this.tramiteService.actualizarUsuario(id, type)
    .subscribe((resp:any)=>{   
      if(resp.data == 0){
        ...
        this.route2.navigate([`tramite-construccion/nuevo-tramite/${btoa(this.folio)}`]).then(r=>{
          this.splash.hide();
        })
      }
  ...
  validarFolio(){
    ...
    request.subscribe((res: any) => {
      ...
      if (res.data.length>0) {
          ...
          this.folioValidado(this.id, res.data[0].typeGoC);
