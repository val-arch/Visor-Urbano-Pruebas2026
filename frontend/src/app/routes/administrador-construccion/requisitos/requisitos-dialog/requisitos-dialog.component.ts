...
import { RequisitosConstruccion } from "../../../../models/administrador/requisitoConstruccion.models";
...
export class RequisitosDialogComponent implements OnInit {
  public modelRequsito: RequisitosConstruccion = new RequisitosConstruccion();
  public requisitoForm: FormGroup;
  public action: string;
  public dialogTitle: string;
  campos: string[] = ['Habitacional', 'Comercial servicios', 'Industrial', 'Alojamiento temporal', 'Equipamiento', 'Espacios verdes', 'Otro'];
  errors = [];
  user:any;
  btnCss = {
  ...
  todasCondiciones;
  dependenciaSelected;
  array_valores_dinamicos = ['', '', '', '', '', '', ''];
  array_condiciones_dinamicos = ['', '', '', '', '', '', ''];
  construccion_uso_result = [];
  construccion_uso_metros_result = [];
  camposDiamicosMunicipioConstruccion = [];
  ...
  ngOnInit(): void {
    ...
    if (this._data.action == "new") {
      ...
      console.log(this._data.requisito);



      if(this._data.requisito.requerido == 2){

        this._data.requisito.construccion_uso = this._data.requisito.construccion_uso.replace(/(['"])/g, '');
        this._data.requisito.construccion_uso = this._data.requisito.construccion_uso.replace(/([[])/g, '');
        this._data.requisito.construccion_uso = this._data.requisito.construccion_uso.replace(']', '');  
        var construccion_uso_array = this._data.requisito.construccion_uso.split(",");
        for(var i = 0; i<construccion_uso_array.length; i++){
          this.construccion_uso_result.push(construccion_uso_array[i]);
        }

        this._data.requisito.construccion_uso_metros = this._data.requisito.construccion_uso_metros.replace(/(['"])/g, '');
        this._data.requisito.construccion_uso_metros = this._data.requisito.construccion_uso_metros.replace(/([[])/g, '');
        this._data.requisito.construccion_uso_metros = this._data.requisito.construccion_uso_metros.replace(']', '');  
        var construccion_uso_metros_array = this._data.requisito.construccion_uso_metros.split(",");
        for(var i = 0; i<construccion_uso_metros_array.length; i++){
          this.construccion_uso_metros_result.push(construccion_uso_metros_array[i]);
        }

        this._data.requisito.construccion_uso_destino = this._data.requisito.construccion_uso_destino.replace(/(['"])/g, '');
        this._data.requisito.construccion_uso_destino = this._data.requisito.construccion_uso_destino.replace(/([[])/g, '');
        this._data.requisito.construccion_uso_destino = this._data.requisito.construccion_uso_destino.replace(/([])/g, '');
        this._data.requisito.construccion_uso_destino = this._data.requisito.construccion_uso_destino.replace(']', '');  
        this._data.requisito.construccion_uso_destino = this._data.requisito.construccion_uso_destino.replace(/\\/, '');
        
        var construccion_uso_destino_array = this._data.requisito.construccion_uso_destino.split(",");

        var construccion_uso_destino_result = [];
        for(var i = 0; i<construccion_uso_destino_array.length; i++){
          construccion_uso_destino_result.push(construccion_uso_destino_array[i]);
          this.asignarDinamicos(construccion_uso_destino_array[i], i);
        }

      }

      this.modelRequsito.step = this.modelRequsito.step.toString();
      this.modelRequsito.pregunta_si_o_no_value = this.modelRequsito.pregunta_si_o_no_value.toString();
      this.getCondiciones();
      ...
      this.getPreguntasSioNo(this.user.id_municipio, this.modelRequsito.tramite_relacionado);
    }

    this._requisitos.getGirosAdmin().subscribe((r) => {
      //  console.log(r);
    ...
    this.requisitoForm = this.fb.group({
      ...
      tramite_relacionado: [this.modelRequsito.tramite_relacionado, Validators.required],
      ...
      info_licencia: [
        this.modelRequsito.info_licencia == null ? false : false,
      ],
      check_alcohol: ["0"],
      propiedad_condicion: [this.propiedad_condicion || "0"],
      propiedad_metros: [this.propiedad_metros],
      actividad_condicion: [this.modelRequsito.actividad_condicion || "0"],
      actividad_metros: [this.modelRequsito.actividad_metros || 0],
      caracter_solicitante_condicion: [
        this.caracter_solicitante_condicion,
      ],
      tipo_persona_condicion: [this.tipo_persona_condicion],
      todasCondiciones: [this.modelRequsito.todasCondiciones ?? "false"],
      dependencias: [Number(this.modelRequsito.condicion_dependencia)],
      Habitacional_condicion: [this.array_condiciones_dinamicos[0] || 0],
      Habitacional_metros: [this.array_valores_dinamicos[0] || ''],
      'Comercial servicios_condicion': [this.array_condiciones_dinamicos[1] || ''],
      'Comercial servicios_metros': [this.array_valores_dinamicos[1] || ''],
      Industrial_condicion: [this.array_condiciones_dinamicos[2] || ''],
      Industrial_metros: [this.array_valores_dinamicos[2] || ''],
      'Alojamiento temporal_condicion': [this.array_condiciones_dinamicos[3] || ''],
      'Alojamiento temporal_metros': [this.array_valores_dinamicos[3] || ''],
      Equipamiento_condicion: [this.array_condiciones_dinamicos[4] ||''],
      Equipamiento_metros: [this.array_valores_dinamicos[4] ||''],
      'Espacios verdes_condicion': [this.array_condiciones_dinamicos[5] ||''],
      'Espacios verdes_metros': [this.array_valores_dinamicos[5] ||''],
      Otro_condicion: [this.array_condiciones_dinamicos[6] ||''],
      Otro_metros: [this.array_valores_dinamicos[6] ||''],
      //construccion_uso :[this.modelRequsito.construccion_uso],
      //construccion_uso_metros :[this.modelRequsito.construccion_uso_metros],
      construccion_uso_destino: [construccion_uso_destino_result || []],
      construccion_total_metros_condicion: [this.modelRequsito.construccion_total_metros_condicion || "0"],
      construccion_total_metros_value: [this.modelRequsito.construccion_total_metros_value || 0],
      demolicion_metros_condicion: [this.modelRequsito.demolicion_metros_condicion || "0"],
      demolicion_metros_value: [this.modelRequsito.demolicion_metros_value || 0],
      viviendas_construccion_condicion: [this.modelRequsito.viviendas_construccion_condicion || "0"],
      viviendas_construccion_value: [this.modelRequsito.viviendas_construccion_value || 0],
      nivel_nuevo_construccion_condicion: [this.modelRequsito.nivel_nuevo_construccion_condicion || "0"],
      nivel_nuevo_construccion_value: [this.modelRequsito.nivel_nuevo_construccion_value || 0],
      sotano_nuevo_construccion_condicion: [this.modelRequsito.sotano_nuevo_construccion_condicion || "0"],
      sotano_nuevo_construccion_value: [this.modelRequsito.sotano_nuevo_construccion_value || 0],
      pregunta_si_o_no_condicion: [this.modelRequsito.pregunta_si_o_no_condicion || "0"],
      pregunta_si_o_no_value: [this.modelRequsito.pregunta_si_o_no_value || 0]

      //Nuevos Campos Construccion
    });
  }
  ...
  getTipoTramites(){
    this.municipioService.getTipoTramites(this.user.id_municipio).subscribe(resp=>{
      ...
      this.mostrarPersonalizar2(this.modelRequsito.tramite_relacionado);
    });
  }
  ...
  guardar() {
    ...
    if (this.errors.length == 0) {
      ...
      let data = {
        ...
        campo_afectado: "",
        tipo_tramite: "",
        tramite_relacionado : 1,
        condicion_dependencia: "",
        condicion_giro: "",
        dependencias: "",
        info_licencia: "",
        construccion_uso_destino: [],
        construccion_uso: [],
        construccion_uso_metros: [],
        construccion_total_metros_condicion: "",
        construccion_total_metros_value: "",
        demolicion_metros_condicion: "",
        demolicion_metros_value: "",
        viviendas_construccion_condicion: "",
        viviendas_construccion_value: "",
        nivel_nuevo_construccion_condicion: "",
        nivel_nuevo_construccion_value: "",
        sotano_nuevo_construccion_condicion: "",
        sotano_nuevo_construccion_value: "",
        actividad_condicion: "",
        actividad_metros: "",
        todasCondiciones: "",
        pregunta_si_o_no_condicion: "",
        pregunta_si_o_no_value: 0
      ...
      if (form.type == "select" || form.type == "radio") {
        if (form.opciones.length > 2) {
          ...
          for (let i = 0; i < form.opciones.length; i++) {
            ...
            if (form.opciones[i] == "") {
              this.errors.push(
                `No puedes dejar el campo número ${i + 1
                } sin texto (Opciones)`
              );
            }
          }
      ...
      var data_condiciones = [];
      var data_metros = [];
      for (let campo of this.campos) {
        if (this.requisitoForm.value[campo + '_condicion'] != undefined && this.requisitoForm.value[campo + '_condicion'] != '') {
          data_condiciones.push(this.requisitoForm.value[campo + '_condicion']);
          data_metros.push(this.requisitoForm.value[campo + '_metros']);
        }
      }
      ...
      data.tramite_relacionado = parseInt(form.tramite_relacionado);
      data.dependencias = form.dependencias;
      data.info_licencia = form.info_licencia;
      data.construccion_uso_destino = form.construccion_uso_destino;
      data.construccion_uso = data_condiciones;
      data.construccion_uso_metros = data_metros;
      data.construccion_total_metros_condicion = form.construccion_total_metros_condicion;
      data.construccion_total_metros_value = form.construccion_total_metros_value;
      data.demolicion_metros_condicion = form.demolicion_metros_condicion;
      data.demolicion_metros_value = form.demolicion_metros_value;
      data.viviendas_construccion_condicion = form.viviendas_construccion_condicion;
      data.viviendas_construccion_value = form.viviendas_construccion_value;
      data.nivel_nuevo_construccion_condicion = form.nivel_nuevo_construccion_condicion;
      data.nivel_nuevo_construccion_value = form.nivel_nuevo_construccion_value;
      data.sotano_nuevo_construccion_condicion = form.sotano_nuevo_construccion_condicion;
      data.sotano_nuevo_construccion_value = form.sotano_nuevo_construccion_value;
      data.actividad_condicion = form.actividad_condicion;
      data.actividad_metros = form.actividad_metros;
      data.todasCondiciones = form.todasCondiciones;
      data.pregunta_si_o_no_condicion = form.pregunta_si_o_no_condicion;
      data.pregunta_si_o_no_value = parseInt(form.pregunta_si_o_no_value);
  ...
  mostrarPersonalizar2(value){
    const result = this.tiposTramite.find(({ id }) => id === value);
    if(result !=undefined){
      this.mostrarPersonalizar = result.default_preguntas;
    }
  }
  ...
  changeTipoTramite(value){
    ...
    if(result !=undefined){
      this.mostrarPersonalizar = result.default_preguntas;
      if(this.requisitoForm.controls['type'].value == 'file'){
        this.getPreguntasSioNo(this.user.id_municipio, this.requisitoForm.controls['tramite_relacionado'].value);
      }
    }else{
      this.mostrarPersonalizar = false;
      if(this.requisitoForm.controls['type'].value == 'file'){
        this.getPreguntasSioNo(this.user.id_municipio, this.requisitoForm.controls['tramite_relacionado'].value);
      }
    }
  ...
  saveRequisito(form) {
    if (this.action == "edit") {
      this.requisitoService.storeRequisitosConstruccion(form).subscribe((resp) => {
        Swal.fire({
          title: "¡Éxito!",
  ...
  setNull(value) {
    if (this.requisitoForm.value[value + '_condicion'] != undefined && this.requisitoForm.value[value + '_condicion'] != '') {
      this.requisitoForm.controls[value + '_condicion'].disable();
    } else {
      this.requisitoForm.controls[value + '_condicion'].enable();
    }
  }

  asignarDinamicos(value, index){
    switch(value){
      case 'Habitacional':
        this.array_condiciones_dinamicos[0] = this.construccion_uso_result[index];
        this.array_valores_dinamicos[0] = this.construccion_uso_metros_result[index];
        break;
      case 'Comercial servicios':
        this.array_condiciones_dinamicos[1] = this.construccion_uso_result[index];
        this.array_valores_dinamicos[1] = this.construccion_uso_metros_result[index];
        break;
      case 'Industrial':
        this.array_condiciones_dinamicos[2] = this.construccion_uso_result[index];
        this.array_valores_dinamicos[2] = this.construccion_uso_metros_result[index];
        break;
      case 'Alojamiento temporal':
        this.array_condiciones_dinamicos[3] = this.construccion_uso_result[index];
        this.array_valores_dinamicos[3] = this.construccion_uso_metros_result[index];
        break;
      case 'Equipamiento':
        this.array_condiciones_dinamicos[4] = this.construccion_uso_result[index];
        this.array_valores_dinamicos[4] = this.construccion_uso_metros_result[index];
        break;
      case 'Espacios verdes':
        this.array_condiciones_dinamicos[5] = this.construccion_uso_result[index];
        this.array_valores_dinamicos[5] = this.construccion_uso_metros_result[index];
        break;
      case 'Otro':
        this.array_condiciones_dinamicos[6] = this.construccion_uso_result[index];
        this.array_valores_dinamicos[6] = this.construccion_uso_metros_result[index];
        break;
  ...
  getRoles(){
    this._role.getAllRoles(this.user.id_municipio).subscribe((r: any) => {
      console.log(r);
        this.rolesList = r;
    },(e) => console.error(e));
  ...
  getPreguntasSioNo(municipio, id){
    this.apivujService.getPreguntasSioNo(municipio, id).subscribe((response: any) => {
