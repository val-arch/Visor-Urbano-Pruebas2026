...
export class MunicipioFormComponent implements OnInit {
  ...
  updateHexValue(event: any) {
    this.municipioForm.patchValue({
      color_hex: event.target.value.toUpperCase() // Actualiza el valor hexadecimal
    });
  }
  ...
  getMunicipio(): void {
    ...
    request.subscribe((res: any) => {
      ...
      this.municipioForm = this._formBuilder.group({
        ...
        url_municipio   :[ this.list['url_municipio']],
        color_hex   :[ this.list['color_hex']],
  ...
  openFile() {
    ...
    console.log(user);
    window.open(`${environment.SERVER_ORIGIN}/licenciaGiroTest/${user.id_municipio    }`, '_blank');
  }
  ...
  getFirmas(){
    let request = this.municipioService.getFirmas(1);
    request.subscribe((res: any) => {
      // console.log(res);
      let d = res.data;
