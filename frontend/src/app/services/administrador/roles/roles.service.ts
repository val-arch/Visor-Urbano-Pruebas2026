...
export class RolesService {
  ...
  getAllRoles(id_municipio) {
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      }),
    };
    return this.http.get(environment.SERVER_ORIGIN + `roles_construccion/getAllRoles/`+id_municipio, httpOptions).pipe(
      tap((respuesta: any) => {
        return respuesta;
      })
    );
  ...
  getUserRoleId() {
    ...
    return this.http.get(environment.SERVER_ORIGIN + `roles_construccion/getRole`, httpOptions).pipe(
      tap((respuesta: any) => {
        return respuesta;
      })
    );
  ...
  deleteRoles(model:Role) {
    ...
    //Servicios Usuario Construccion 

    storeRolesConstruccion(model:Role) {
      if (model.id) {
        const httpOptions = {
            headers: new HttpHeaders({
              'Authorization': this._token.get().token,
        ...
        return this.http.put<Role[]>(environment.SERVER_ORIGIN +`roles_construccion/${model.id}`,model,httpOptions);
      }else{ 
        delete model.id;  
        const httpOptions = {
          headers: new HttpHeaders({
        ...
        return this.http.post<Role[]>(environment.SERVER_ORIGIN +`roles_construccion`,model,httpOptions);
      }
    }
