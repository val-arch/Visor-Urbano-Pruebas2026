...
export class RequisitosService {
  ...
  getRequisitosConstruccion(page: number, filter: string) {
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      }),
    };
    return this.http.get(environment.SERVER_ORIGIN + `campos_construccion?page=${page}&filter=${filter}`, httpOptions).pipe(
      tap((respuesta: any) => {
        return respuesta;
      })
    );
  ...
  postRequisitoChangeConstruccion(id_campo, id_requisito = 0) {
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      }),
    ...
    return this.http.post<any[]>(environment.SERVER_ORIGIN + `campos_construccion/requisitoChange`, data, httpOptions);

  }

  hideRequisitoConstruccion(id) {
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      }),
    };
    return this.http.put<any[]>(environment.SERVER_ORIGIN + `campos_construccion/hideRequisito/${id}`, {mostrar:false}, httpOptions);

  }

  // Servicios Construccion 

  storeRequisitosConstruccion(model) {
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      }),
    ...
    if (model.id) {
      delete model.name;
      return this.http.put<any[]>(environment.SERVER_ORIGIN + `campos_construccion/${model.id}`, model, httpOptions);
    } else {
      return this.http.post<any[]>(environment.SERVER_ORIGIN + `campos_construccion`, model, httpOptions);
    }
  }
