...
export class UseradminService {
  ...
  getUsers4(page: number,filter:string) {
    const httpOptions = {
      headers: new HttpHeaders({
        'Authorization': this._token.get().token,
      }),
    };
    return this.http.get(environment.SERVER_ORIGIN + `usuarios/getByRole?page=${page}&filter=${filter}`, httpOptions).pipe(
      tap((respuesta: any) => {
        return respuesta;
      })
    );
...
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
getMunicipiosConstruccion() {
  const httpOptions = {
    headers: new HttpHeaders({
      'Authorization': this._token.get().token,
    }),
  };
  return this.http.get(environment.SERVER_ORIGIN + `municipios_construccion`, httpOptions).pipe(
    tap((respuesta: any) => {
      return respuesta;
    })
  );
}
getUsers3(page: number,filter:string) {
  const httpOptions = {
    headers: new HttpHeaders({
      'Authorization': this._token.get().token,
    }),
  };
  return this.http.get(environment.SERVER_ORIGIN + `usuarios/getByTypeUser?page=${page}&filter=${filter}`, httpOptions).pipe(
    tap((respuesta: any) => {
      return respuesta;
    })
  );
}
storeUsersConstruccion(model:UserAdmin) {
  if (model.id) {
    const httpOptions = {
        headers: new HttpHeaders({
          'Authorization': this._token.get().token,
    ...
    return this.http.post<UserAdmin[]>(environment.SERVER_ORIGIN +`usuarios/storeByUserType`,model,httpOptions);
  }
}
