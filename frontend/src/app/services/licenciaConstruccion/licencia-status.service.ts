...
export class LicenciaStatusService {
  ...
  getInfoLicencia(id): Observable<any[]> { 
    ...
    getLicenciasEmitidas(page: number,filter:string) {
      ...
      return this.http.get(environment.SERVER_ORIGIN + `licencias_construccion/listado-licencias?page=${page}&filter=${filter}`, httpOptions).pipe(
        tap((respuesta: any) => {
          return respuesta;
        })
      );
    ...
    enviarLicenciaInteresados(folio:any){
      ...
      return this.http.post(environment.SERVER_ORIGIN + `licencias_construccion/enviarLicenciaInteresados/`+folio, {}, httpOptions).pipe(
        tap((respuesta: any) => {
          return respuesta;
        })
      );
