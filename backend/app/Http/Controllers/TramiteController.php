...
class TramiteController extends Controller
{
    ...
    public function ingreso(Request $request){
        ...
        $this->historial($accion='Se Valido  Captura de tramite',$valor_anterior,$datos2,$tipo=2,0);
        return $this->tramiteRepository->ingreso($folio);

    }
    public function ingresoRefrendo(Request $request){
        $datos =$request->all();
        $datos2 =json_encode($datos);
        $folio =  base64_decode($datos['folio']);
        ...
        $valor_anterior = $folio;
        $this->historial($accion='Se Valido  Captura de tramite',$valor_anterior,$datos2,$tipo=2,0);
        return $this->tramiteRepository->ingresoRefrendo($folio);
    ...
    public function noFirmaElectronica(Request $request, $folio){
        ...
        $valor_anterior = $folio;
        $this->historial($accion='Finalizo Captura de tramite',$valor_anterior,$datos2,$tipo=3,0);
    ...
    public function ingresoTramiteContinuar(Request $request, $folio){
        ...
        $valor_anterior =  $folio;
        $this->historial($accion='Se envio la solicitud del tramite',$valor_anterior,$datos2,$tipo=2,0);
    ...
    public function getListadoDesechados(Request $request){
        ...
        //  return $folio;
        $data = $this->tramiteRepository->getListadoDesechados($folio);
        if($data == false){
            return $this->errorResponse('No tienes permiso', 403);  
        }else{
            return $this->successResponse($data);
