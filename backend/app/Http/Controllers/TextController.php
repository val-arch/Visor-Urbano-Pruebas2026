...
class TextController extends Controller
{
  ...
  public function guardarTextId(Request $request)
  {
    ...
    $this->historial($accion = 'Se registro un nuevo step', $valor_anterior, $data2, $tipo = 2, 0);
      ...
      foreach ($test as $key => $value) {
        ...
        if ($key != 'folio' || $key != 'step_actual') {
          ...
          if ($respuesta === null) {
            ...
            DB::table('respuestas')->where('name', '=', $key)
              ->where('id_tramite', '=', $idConsulta)->update($data);

            DB::table('respuestas_json')
  ...
  public function guardarTextRefrendo(Request $request)
  {
    ...
    $id_requisito = $RefrendoTramite->id_consulta_requisitos;
    $query = User::select('user_roles.id', 'user_roles.role_id')
      ->join('user_roles', 'users.id', '=', 'user_roles.user_id')
      ->where('user_id', '=',  $usuarioActual_id)
      ->get();
    ...
    if ($role > 1 || isset($usuarioFolio) && ($role == 1 && $usuarioActual_id == $usuarioFolio)) {
      ...
      if ($step_actual == 1) {
        $tramite = Tramite::where('id_consulta_requisitos',    $id_requisito)->first();
        $tramite->step_uno =  1;
        $tramite->step_actual =  $step_actual;
        $tramite->save();
      }
  ...
  public function guardarHistoricoRefrendo(Request $request)
  {
    ...
    if ($datos['step_actual'] == 4) {
      ...
      $historico->anuncio = $datos['anuncio']  == 'null' ? '' : $datos['anuncio'];
      }
