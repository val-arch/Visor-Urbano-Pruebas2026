...
class userP extends Seeder{
    ...
    public function run()
    {
        ...
        foreach ($muni as $municipio) {
            ...
            $d1 = array('á','Á','é','É','í','Í','ó','Ó','ú','Ú');
            $d2 = array('a','A','e','E','i','I','o','O','u','U');
            $nombre = str_replace($d1,$d2,$nombre);
            $nombre = strtolower($nombre);
            $this->newUserRole($municipio->id,"director".$nombre."@visorurbano.com","visorUrbanoJalisco2021".$nombre."$municipio->id.", str_replace(' ','-',$municipio->nombre),4);
        }
